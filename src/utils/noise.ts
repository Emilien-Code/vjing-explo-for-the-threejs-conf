...
function SimplexNoise(random) {
    ...
    for (var i = 0; 256 > i; i++) this.p[i] = 256 * Math.random();
...
const generate = (random = Math.random) => {
    noise = new SimplexNoise(random)
}

export default getNoise
export { generate, SimplexNoise, curlNoise3D, noisePerlin }