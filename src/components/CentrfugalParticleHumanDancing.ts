...
export default class ParticleHumanDancing extends World {
    ...
    private createGPGPU() {
        ...
        for (let i = 0; i < this.baseGeometry.count; i++) {
            ...
            baseParticlesTexture.image.data[i4 + 2] = 0//this.baseGeometry.instance.attributes.position.array[i3 + 2]
        ...
        // Previous frame's targets start identical — no delta on first frame
...
${simplex4DNoise}
        ...
        void main()
        {
            ...
            // Distance to real bone target — used by vertex shader for size boost
        ...
        this.gpgpu.particlesVariable.material.uniforms.uModelCenter = { value: this.params.offset.clone() }
    ...
    private setupGUI() {
        ...
        folder.add(this.params, 'damping', 0.1, 30, 0.1)
            .name('Follow Speed')
            .onChange((v: number) => {
                this.gpgpu.particlesVariable.material.uniforms.uDamping.value = v
            })

        folder.add(this.params, 'centrifugalFactor', 0, 20, 0.1)
            .name('Centrifugal')
            .onChange((v: number) => {
                this.gpgpu.particlesVariable.material.uniforms.uCentrifugalFactor.value = v
            })
    ...
    update() {
        ...
        for (let i = 0; i < this.baseGeometry.count; i++) {
            ...
            this.targetPositionsTexture.image.data[i4 + 0] = tempPos.x + this.particleScatterOffset[i3 + 0] * shellRadius + this.params.offset.x
            this.targetPositionsTexture.image.data[i4 + 1] = tempPos.y + this.particleScatterOffset[i3 + 1] * shellRadius + this.params.offset.y
            this.targetPositionsTexture.image.data[i4 + 2] = 0 //tempPos.z + this.particleScatterOffset[i3 + 2] * shellRadius + this.params.offset.z
            this.targetPositionsTexture.image.data[i4 + 3] = 0
        }

        // Upload prev targets to GPU texture
        ...
        this.gpgpu.particlesVariable.material.uniforms.uDeltaTime.value = dt
        this.gpgpu.particlesVariable.material.uniforms.uTargetPositions.value = this.targetPositionsTexture
        this.gpgpu.particlesVariable.material.uniforms.uPrevTargetPositions.value = this.prevTargetPositionsTexture
        this.gpgpu.particlesVariable.material.uniforms.uModelCenter.value.copy(this.params.offset)
